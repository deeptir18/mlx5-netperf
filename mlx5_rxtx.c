		      ...
		      bool zero_copy)
{
	...
	if (nr_inflight_tx(v) >= SQ_CLEAN_THRESH) {
	  compl = mlx5_gather_completions(mbs, v, SQ_CLEAN_MAX);
	  NETPERF_DEBUG("Gathered completions: %d", compl);
	  for (i = 0; i < compl; i++) {
	    NETPERF_DEBUG("freeing mbuf %d\n");
	    if ( zero_copy ) {
			...
			bool zero_copy)
{
    ...
    for (size_t i = start_index; i < burst_size; i++) {
        ...
        if (mlx5_transmit_one(mbuf, v, request_headers[i], inline_len[i],
			      tx_buf_mempool, mbuf_mempool, zero_copy) == 0) {
            return total_sent;
        }
        total_sent += 1;
    }
