...
int server_init_refcnt_array(size_t num_items) {
    ...
    for (size_t j = 0; j < num_refcnt_arrays; j++) {
        void *refcnts;
        refcnts = malloc(sizeof(uint16_t) * num_items);
        if (refcnts == NULL) {
            NETPERF_DEBUG("Malloc failed: resulting refcnts buffer is null");
            errno = -ENOMEM;
            return ENOMEM;
        }
        working_set_refcnts[j] = refcnts;
    }

    NETPERF_INFO("allocated refcnts");
    NETPERF_INFO("Set refcnts to array addr");
    for (size_t j = 0; j < num_refcnt_arrays; j++) {
        for (size_t i = 0; i < num_items; i++) {
            working_set_refcnts[j][i] = 1;
    }}
    return 0;
}
...
int mlx5_qs_init_flows(struct mlx5_rxq **v,
  ...
  struct ibv_rwq_ind_table_init_attr rwq_attr = {
    .log_ind_tbl_size = __builtin_ctz(SIZE),
    .ind_tbl = ind_tbl,
    .comp_mask = 0,
  };

  struct ibv_rwq_ind_table* rwq_ind_table;
  rwq_ind_table = ibv_create_rwq_ind_table(ibv_context, &rwq_attr);
  if (!rwq_ind_table) {
  ...
  struct ibv_qp* qp;
  qp = ibv_create_qp_ex(ibv_context, &qp_ex_attr);
  if (!qp) {
    NETPERF_WARN("Failed to create rx qp");
    return -errno;
  }

  /** Register steering rules to intercept packets to our mac address and
